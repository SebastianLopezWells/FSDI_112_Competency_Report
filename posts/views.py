...
class DraftPostListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        pending_status = Satus.objects.get(name="draft")
...
class PublishedPostListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        pending_status = Satus.objects.get(name="published")
        context["post_list"] = Post.objects.filter(
            author=self.request.user).filter(status=pending_status).order_by('created_on').reverse()
        return context
...
class ArchivedPostListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        pending_status = Satus.objects.get(name="archived")
        context["post_list"] = Post.objects.filter(
            author=self.request.user).filter(status=pending_status).order_by('created_on').reverse()
        return context
